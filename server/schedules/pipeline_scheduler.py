...
class PipelineScheduler:
    def __init__(self, pipeline_server: _PipelineServer):
        self.db_client = get_pipeline_db_client().schedules
    ...
    async def initialize(self):
        await self._restart_saved_schedules()

    async def _restart_saved_schedules(self):
        async for schedule_entity in self.db_client.find():
    ...
    async def add_schedule(self, schedule_creation: ScheduleCreation, user: UserDto) -> PipelineSchedule:
        ...
        await self.db_client.insert_one(schedule.to_entity())
        if schedule.cron and schedule.active:
            self.scheduler.add_schedule(
                self.run,
                CronTrigger.from_crontab(schedule.cron),
    ...
    async def get_schedules(
        ...
        total_records = await self.db_client.count_documents(query)
        return PaginatedListDto(
            [PipelineSchedule.from_entity(schedule) async for schedule in schedules], PageDto(offset, limit, total_records)
        )

    async def get_schedule(self, schedule_id: str) -> PipelineSchedule:
        schedule = await self.db_client.find_one({"_id": ObjectId(schedule_id)})
        if not schedule:
    ...
    async def update_schedule(self, schedule_id: str, schedule_update: ScheduleCreation, user: UserDto) -> PipelineSchedule:
        ...
        await self.db_client.replace_one({"_id": ObjectId(schedule_id)}, schedule.to_entity())
        if schedule.cron and schedule.active:
            self.scheduler.add_schedule(
    ...
    async def run(self, schedule_id: str, user: Optional[UserDto] = None) -> Pipeline:
        ...
        pipeline = await self.pipeline_server.add_pipeline(
            pipeline_config,
            schedule.pipeline,
